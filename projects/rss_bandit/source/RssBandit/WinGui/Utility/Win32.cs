...
namespace RssBandit
{
	...
	internal static class Win32
	{
		...
		private static readonly ILog _log = Log.GetLogger(typeof(Win32));
		...
		private static readonly NativeMethods.SYSTEM_INFO _systemInfo;
		private static readonly NativeMethods.OSVERSIONINFOEX _osvi;
		...
		static Win32()
		{
			...
			_osvi = new NativeMethods.OSVERSIONINFOEX();
			_osvi.dwOSVersionInfoSize = Marshal.SizeOf(_osvi);
			if (!NativeMethods.GetVersionEx(ref _osvi))
			{
			...
			else
			{
				...
				_systemInfo = new NativeMethods.SYSTEM_INFO();
				IntPtr pGNSI = NativeMethods.GetProcAddress(NativeMethods.GetModuleHandle("kernel32.dll"), "GetNativeSystemInfo");
				if (IntPtr.Zero != pGNSI)
				{
					var invoker =
						(NativeMethods.GetNativeSystemInfoInvoker)
							Marshal.GetDelegateForFunctionPointer(pGNSI, typeof(NativeMethods.GetNativeSystemInfoInvoker));
					invoker(ref _systemInfo);
				}
				else
					NativeMethods.GetSystemInfo(ref _systemInfo);
				...
				if (_os.Version.Major == 6)
				{
					...
					{
						if (_osvi.wProductType == NativeMethods.VER_NT_WORKSTATION)
						{
							IsVista = true;
				...
				{
					IsWin2k3 = true;
					if (0 != NativeMethods.GetSystemMetrics(NativeMethods.SystemMetric.SM_SERVERR2))
						dispStr.Append("Windows Server 2003 R2, ");
					else if (GetWindowsSuiteInfo() == NativeMethods.WindowsSuite.VER_SUITE_STORAGE_SERVER)
						dispStr.Append("Windows Storage Server 2003");
					else if (GetWindowsSuiteInfo() == NativeMethods.WindowsSuite.VER_SUITE_WH_SERVER)
						dispStr.Append("Windows Home Server");
					else if (_osvi.wProductType == NativeMethods.VER_NT_WORKSTATION &&
							 _systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_AMD64)
					{
					...
					if (_osvi.wProductType != NativeMethods.VER_NT_WORKSTATION)
					{
						if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_IA64)
						{
							if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_DATACENTER))
								dispStr.Append("Datacenter Edition for Itanium-based Systems");
							else if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_ENTERPRISE))
								dispStr.Append("Enterprise Edition for Itanium-based Systems");
						}

						else if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_AMD64)
						{
							if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_DATACENTER))
						...
						{
							if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_COMPUTE_SERVER))
								dispStr.Append("Compute Cluster Edition");
							else if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_DATACENTER))
								dispStr.Append("Datacenter Edition");
							else if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_ENTERPRISE))
								dispStr.Append("Enterprise Edition");
							else if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_BLADE))
				...
				if ((_os.Version.Major == 5) && (_os.Version.Minor == 1))
				{
					...
					if (IsWindowsSuite(NativeMethods.WindowsSuite.VER_SUITE_PERSONAL))
			...
			if (_os.Version.Major >= 6)
			{
				if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_AMD64)
					dispStr.Append(", 64-bit");
				else if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_INTEL)
			...
			try
			{
				...
				switch (applicationSound)
				{
					case Resource.ApplicationSound.FeedDiscovered:
						NativeMethods.PlaySound(applicationSound, IntPtr.Zero, NativeMethods.SoundFlags.SND_APPLICATION |
							NativeMethods.SoundFlags.SND_NOWAIT | NativeMethods.SoundFlags.SND_NODEFAULT);
						break;
		...
		/// <summary>
		/// Wrap the windows registry read access required for Bandit
		/// </summary>
		internal abstract class ReadOnlyWindowsRegistry
		{
			/// <summary>
			/// Gets the internet explorer version.
			/// </summary>
			/// <returns></returns>
			protected static Version GetRegistryInternetExplorerVersion()
			{
		...
		/// </summary>
		internal class WindowsRegistry : ReadOnlyWindowsRegistry, IRegistry
		{

			private static readonly log4net.ILog _log = Log.GetLogger(typeof(Registry));
			...
			bool IRegistry.IsInternetExplorerExtensionRegistered(IEMenuExtension extension)
			{
				...
				if (keyName != null)
				{
					...
					{
						try
						{	// delete bad entries (pointing to a missing file)
			...
			private static void WriteIEExtensionScript(IEMenuExtension extension)
			{
				...
				try
				{
					...
					if (scriptContent != null)
					{
						...
						{	// set the path to the exe within script
		...
		/// </summary>
		private class PortableRegistry : ReadOnlyWindowsRegistry, IRegistry
		{
		...
		public static ProcessorArchitecture ProcessorArchitecture
		{
			get
			{
				...
				if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_INTEL)
					return ProcessorArchitecture.X86;
				if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_IA64)
					return ProcessorArchitecture.IA64;
				if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_AMD64)
					return ProcessorArchitecture.Amd64;
				if (_systemInfo.processorArchitecture == NativeMethods.PROCESSOR_ARCHITECTURE_UNKNOWN)
		...
		internal static NativeMethods.WindowsSuite GetWindowsSuiteInfo()
		{
			return (NativeMethods.WindowsSuite)_osvi.wSuiteMask;
		}
		/// <summary>
		...
		/// </returns>
		internal static bool IsWindowsSuite(NativeMethods.WindowsSuite info)
		{
			return ((NativeMethods.WindowsSuite)_osvi.wSuiteMask & info) == info;
		}
		...
		/// <returns></returns>
		public static string WindowsSuiteToString(NativeMethods.WindowsSuite suite)
		{
			var sb = new StringBuilder();
			if ((suite & NativeMethods.WindowsSuite.VER_SERVER_NT) != 0)
				sb.Append("NT Server, ");
			if ((suite & NativeMethods.WindowsSuite.VER_WORKSTATION_NT) != 0)
				sb.Append("NT WorkStation, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_SMALLBUSINESS) != 0)
				sb.Append("Small Business Server, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_SMALLBUSINESS_RESTRICTED) != 0)
				sb.Append("Small Business Server (restricted license), ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_ENTERPRISE) != 0)
			{
				if (IsWin2K)
			...
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_BACKOFFICE) != 0)
				sb.Append("BackOffice installed, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_COMMUNICATIONS) != 0)
				sb.Append("Communications, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_TERMINAL) != 0)
				sb.Append("Terminal Services installed, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_SINGLEUSERTS) != 0)
				sb.Append("Terminal Service installed (one interactive session only), ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_EMBEDDEDNT) != 0)
				sb.Append("Embedded NT, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_EMBEDDED_RESTRICTED) != 0)
				sb.Append("Embedded (restricted), ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_DATACENTER) != 0)
			{
				if (IsWin2K)
			...
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_PERSONAL) != 0)
				sb.Append("Home Edition, ");
			if ((suite & NativeMethods.WindowsSuite.VER_SUITE_BLADE) != 0)
		...
		public static string GetProductInfo()
		{
			if (_os.Version.Major == 6)
			{
				IntPtr pGpi = NativeMethods.GetProcAddress(NativeMethods.GetModuleHandle("kernel32.dll"), "GetProductInfo");
				if (pGpi != IntPtr.Zero)
				{
					var invoker = (NativeMethods.GetProductInfoInvoker)Marshal.GetDelegateForFunctionPointer(pGpi, typeof(NativeMethods.GetProductInfoInvoker));
					uint dwType;
					if (invoker(_os.Version.Major, _os.Version.Minor, 0, 0, out dwType))
						return NativeMethods.WindowsProductType.Get(dwType);
				}
			}
			return String.Empty;
		}
		...
		{
			NativeMethods.API_STARTUPINFO sti = new NativeMethods.API_STARTUPINFO();
			try
			{
				sti.cb = Marshal.SizeOf(typeof(NativeMethods.API_STARTUPINFO));
				NativeMethods.GetStartupInfo(ref sti);
				...
				if (sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_MINIMIZE || sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_SHOWMINIMIZED ||
					sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_SHOWMINNOACTIVE || sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_FORCEMINIMIZE)
					return FormWindowState.Minimized;
				if (sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_MAXIMIZE || sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_SHOWMAXIMIZED ||
					sti.wShowWindow == (short)NativeMethods.ShowWindowStyles.SW_MAX)
		...
		public static void FreezePainting(Control ctrl, bool freeze)
		{
			...
			if (freeze && ctrl != null && ctrl.IsHandleCreated && ctrl.Visible)
			{
				if (0 == _paintFrozen++)
				{
					NativeMethods.SendMessage(ctrl.Handle, (int)NativeMethods.Message.WM_SETREDRAW, 0, IntPtr.Zero);
				}
			}
			if (!freeze)
			{
				...
				{
					NativeMethods.SendMessage(ctrl.Handle, (int)NativeMethods.Message.WM_SETREDRAW, 1, IntPtr.Zero);
		...
		internal class NativeMethods
		{
			...
			/// </summary>
			public static class WindowsProductType
			{
				#region consts
				...
				internal static string Get(uint type)
				{
					switch (type)
					{
						...
						case PRODUCT_DATACENTER_SERVER:
							return "Server Datacenter Edition (full installation)";
						case PRODUCT_DATACENTER_SERVER_CORE:
							return "Server Datacenter Edition (core installation)";
						case PRODUCT_DATACENTER_SERVER_CORE_V:
							return "Server Datacenter Edition without Hyper-V (core installation)";
						case PRODUCT_DATACENTER_SERVER_V:
							return "Server Datacenter Edition without Hyper-V (full installation)";
						case PRODUCT_ENTERPRISE:
							return "Enterprise Edition";
						case PRODUCT_ENTERPRISE_E:
							return "Enterprise Edition E";
						case PRODUCT_ENTERPRISE_N:
							return "Enterprise Edition N";
						case PRODUCT_ENTERPRISE_SERVER:
							return "Server Enterprise Edition (full installation)";
						case PRODUCT_ENTERPRISE_SERVER_CORE:
							return "Server Enterprise Edition (core installation)";
						case PRODUCT_ENTERPRISE_SERVER_CORE_V:
							return "Server Enterprise Edition without Hyper-V (core installation)";
						case PRODUCT_ENTERPRISE_SERVER_IA64:
							return "Server Enterprise Edition for Itanium-based Systems";
						case PRODUCT_ENTERPRISE_SERVER_V:
							return "Server Enterprise Edition without Hyper-V (full installation)";
						case PRODUCT_HOME_BASIC:
							return "Home Basic Edition";
						case PRODUCT_HOME_BASIC_E:
							return "Home Basic Edition E";
						case PRODUCT_HOME_BASIC_N:
							return "Home Basic Edition N";
						case PRODUCT_HOME_PREMIUM:
							return "Home Premium Edition";
						case PRODUCT_HOME_PREMIUM_E:
							return "Home Premium Edition E";
						case PRODUCT_HOME_PREMIUM_N:
							return "Home Premium Edition N";
						case PRODUCT_HYPERV:
							return "Microsoft Hyper-V Server Edition";
						...
						case PRODUCT_PROFESSIONAL_E:
							return "Professional Edition E";
						case PRODUCT_PROFESSIONAL_N:
							return "Professional Edition N";
						...
						case PRODUCT_SERVER_FOUNDATION:
							return "Server Foundation Edition";
						case PRODUCT_SMALLBUSINESS_SERVER:
							return "Windows Small Business Server";
						case PRODUCT_STANDARD_SERVER:
							return "Server Standard Edition (full installation)";
						case PRODUCT_STANDARD_SERVER_CORE:
							return "Server Standard Edition (core installation)";
						case PRODUCT_STANDARD_SERVER_CORE_V:
							return "Server Standard Edition without Hyper-V (core installation)";
						case PRODUCT_STANDARD_SERVER_V:
							return "Server Standard Edition without Hyper-V (full installation)";
						case PRODUCT_STARTER:
							return "Starter Edition";
						case PRODUCT_STARTER_E:
							return "Starter Edition E";
						case PRODUCT_STARTER_N:
							return "Starter Edition N";
						...
						case PRODUCT_ULTIMATE_E:
							return "Ultimate Edition E";
						case PRODUCT_ULTIMATE_N:
							return "Ultimate Edition N";
						case PRODUCT_WEB_SERVER:
							return "Web Server Edition (full installation)";
			...
			public delegate void GetNativeSystemInfoInvoker(ref SYSTEM_INFO lpSystemInfo);

			public delegate bool GetProductInfoInvoker(int dwOSMajorVersion,
			...
			[Flags]
			internal enum ShellFileInfoFlags : uint
			{
				Icon = 0x100,
				LargeIcon = 0x0,
				SmallIcon = 0x1,
