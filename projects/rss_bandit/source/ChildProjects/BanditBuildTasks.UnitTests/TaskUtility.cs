...
namespace BanditBuildTasks.UnitTests
{
    public class TaskUtility
    {
		...
		/// </summary>
		/// <value>The test directory.</value>
        ...
        public static string getProjectRootDirectory(bool assertIgnoreOnFailure)
        {
            ...
            string searchPath = (CalledInBuildDirectory) ? @"build\" : @"source\";
            int foundIndex = testDir.IndexOf(searchPath, StringComparison.OrdinalIgnoreCase);
            if (foundIndex < 0)
            {
                if (assertIgnoreOnFailure)
                {
                    Assert.Ignore(String.Format("Can't find '{0}' in '{1}'", searchPath, testDir));
                }
                else
                {
                    return null;
        ...
        public static Version GetInstalledIISVersion(string machineName)
        {
            int majorVersion = 0;
            int minorVersion = 0;
            const string iisRegKeyName = @"SYSTEM\CurrentControlSet\Services\W3SVC\Parameters";
		...
		/// <param name="iisMajorVersion">The IIS major version.</param>
		/// <param name="iisMinorVersion">The IIS minor version.</param>
    	...
    	{
            Version installedVersion = GetInstalledIISVersion(machineName);
            return installedVersion.CompareTo(new Version(iisMajorVersion, iisMinorVersion)) >= 0;
		}

		/// <summary>
		/// Determines whether [is admin on remote machine] [the specified machine name].
		/// </summary>
		/// <param name="machineName">Name of the machine.</param>
		/// <returns>
		/// 	<c>true</c> if [is admin on remote machine] [the specified machine name]; otherwise, <c>false</c>.
		/// </returns>
